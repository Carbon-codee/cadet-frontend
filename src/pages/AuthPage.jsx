...
const AuthPage = () => {
    ...
    const handleLoginSubmit = async (e) => {
        ...
        try {
            ...
            // 1. Context'i güncelle
            login(data);

            // 2. KESİN YÖNLENDİRME: Her zaman Ana Sayfaya git!
            // { replace: true } geçmişi siler, böylece 'Geri' tuşuna basınca tekrar login'e dönmez.
                ...
                <div className="auth-header">
                    ...
                    {!isLogin && (
                        <div className="role-selector">
                            <button className={role === 'student' ? 'active' : ''} onClick={() => setRole('student')}><FaUser /> Öğrenci</button>
                            <button className={role === 'company' ? 'active' : ''} onClick={() => setRole('company')}><FaBuilding /> Şirket</button>
                            <button className={role === 'lecturer' ? 'active' : ''} onClick={() => setRole('lecturer')}><FaChalkboardTeacher /> Akademisyen</button>
                        </div>
                    )}
                </div>
